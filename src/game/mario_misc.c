...
Gfx *geo_mario_change_pfp(s32 callContext, struct GraphNode *node, UNUSED Mat4 *mtx) {
    Gfx *gfx = NULL;

    if (callContext == GEO_CONTEXT_RENDER) {
       struct GraphNodeGenerated *this = (struct GraphNodeGenerated *) node;

       SET_GRAPH_NODE_LAYER(this->fnNode.node.flags, LAYER_ALPHA);

       Gfx *head = alloc_display_list(3 * sizeof(Gfx));
       gfx = head;

       gDPSetTextureImage(head++, G_IM_FMT_RGBA, G_IM_SIZ_32b_LOAD_BLOCK, 1, rhdcPFPRGBA32);
       gSPDisplayList(head++, mario_head_tex_end);
