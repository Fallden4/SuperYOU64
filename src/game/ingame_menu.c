...
void render_intro_text(void) {
    ...
    if (font_size < 150) { 
        font_size = 150; // min size
    } else if (font_size > 400) {
        font_size = 400; // max size
    }
    sprintf(msg, "<SIZE_%d>%s", font_size, username);
    print_small_text(SCREEN_CENTER_X, SCREEN_CENTER_Y, msg, PRINT_TEXT_ALIGN_CENTER, 32, FONT_OUTLINE);
...
void render_dialog_entries(void) {
    ...
    gDialogVariable.asStr = username;
...
void do_cutscene_handler(void) {
    ...
    char EndCutsceneStringMSG[256];


    sprintf(EndCutsceneStringMSG, LANG_ARRAY(*gEndCutsceneStringsEn[gCutsceneMsgIndex]), username);


    print_generic_string_aligned(SCREEN_CENTER_X, 13, EndCutsceneStringMSG, TEXT_ALIGN_CENTER);
