...
static void get_rhdc_info(void) {
    if (!isLibpl || !libpl_is_supported(LPL_ABI_VERSION_3)) {
        isLibpl = FALSE;
        error("You need to be using Parallel Launcher!");
   }
    if (!usernameSet) {
        const char *res = libpl_get_my_rhdc_username();
        if (!res) {
             error("You need to be logged into RHDC!");
        }
        bcopy(res, username, sizeof(username));
        usernameSet = TRUE;
    }
    if (usernameSet && !rhdcPFPSet) {
        u8 ret = libpl_get_rhdc_avatar_32_async(username, rhdcPFPRGBA32);
